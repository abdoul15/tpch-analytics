...
def test_end_to_end_pipeline(spark, sample_customer_data, sample_nation_data, 
                            sample_region_data, sample_orders_data, 
                            sample_lineitem_data, sample_part_data,sample_partsupp_data, sample_supplier_data,sample_wide_order_details ,temp_storage_dir):
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        class WideOrderDetailsGoldWrapper(WideOrderDetailsGoldETL):
            def __init__(self, spark, **kwargs):
                super().__init__(spark=spark, storage_path=wide_order_details_path, **kwargs)
        
        daily_sales_metrics = DailySalesMetricsGoldETL(
            spark=spark,
            upstream_table_names=[
                WideOrderDetailsGoldWrapper
            ],
        ...
        expected_columns = [
            'Date', 'Market Segment', 'Region', 'Total Revenue',
            'Average Order Value', 'Order Count', 'Late Delivery %',
            'Fulfillment Rate %'
        ]
        ...
        unique_combinations = (
            ...
            .select('order_date', 'market_segment', 'customer_region')
            .distinct()
            .count()
        )
