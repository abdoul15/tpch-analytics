...
def test_daily_sales_metrics_transform(spark, sample_wide_order_details):
    ...
    with patch.object(DailySalesMetricsGoldETL, 'extract_upstream') as mock_extract:
        ...
        result = daily_sales_metrics_etl.transform_upstream(daily_sales_metrics_etl.extract_upstream())
        ...
        expected_columns = [
            'date', 'market_segment', 'customer_region', 
            'total_sales', 'total_discounts', 'average_order_value',
            'number_of_orders', 'number_of_items', 'avg_shipping_delay',
            'avg_delivery_delay', 'late_delivery_percentage',
            'unique_customers', 'unique_products', 'fulfillment_rate',
            'etl_inserted'
        ]
...
def test_daily_sales_metrics_read(spark, sample_wide_order_details):
    ...
    with patch.object(DailySalesMetricsGoldETL, 'extract_upstream') as mock_extract:
        ...
        daily_sales_metrics_etl.transform_upstream(daily_sales_metrics_etl.extract_upstream())
