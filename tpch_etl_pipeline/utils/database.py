...
def get_table_from_db(
    ...
    if num_partitions is None:
        if size_info and size_info['size']:
            ...
            num_partitions = max(1, int(size_bytes / (target_partition_size_mb * 1024 * 1024)))
        else:
            num_partitions = 1  # Fallback

    # 3. Lecture avec partitionnement JDBC
    if partition_column:
        query = f'(SELECT MIN({partition_column}) as min, MAX({partition_column}) as max FROM {table_name}) as tmp'
        bounds = spark.read.jdbc(url=jdbc_url, table=query, properties=base_properties).first()
