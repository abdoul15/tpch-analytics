    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        supply_chain_metrics = (
            ...
            .agg(
                ...
                round(stddev('shipping_delay_days'), 1).alias(
                    'shipping_delay_variability'
                ),
                round(
                ...
                round(
                    sum(when(col('order_status') == 'F', 1).otherwise(0))
                    / count('*')
                    * 100,
                    2,
                ).alias('order_fulfillment_rate'),
                # Délai moyen de traitement des commandes (jours)
                round(avg(datediff(current_date(), col('order_date'))), 1).alias(
