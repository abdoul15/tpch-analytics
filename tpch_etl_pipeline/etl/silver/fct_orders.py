...
class FctOrdersSilverETL(TableETL):
    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        transformed_data = (
            ...
            # Calcul des délais
            .withColumn(
                'shipping_delay_days', datediff(col('ship_date'), col('order_date'))
            )
            .withColumn(
                'delivery_delay_days', datediff(col('receipt_date'), col('ship_date'))
            )
            # Indicateurs de performance
            .withColumn(
                'is_late_delivery',
                when(col('receipt_date') > col('commit_date'), True).otherwise(False),
            )
